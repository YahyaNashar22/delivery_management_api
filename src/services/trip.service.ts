...
import { ITripDocument, TripModel } from "../models/trip.model.js";
...
type TripSettlement = {
  ...
  returned_with_fee_count: number;
  returned_without_fee_count: number;
  order_total_usd: number;
  order_total_lbp: number;
  fee_collected_usd: number;
  fee_collected_lbp: number;
  collected_usd: number;
  collected_lbp: number;
  refund_usd: number;
  refund_lbp: number;
  net_usd: number;
  net_lbp: number;
...
export class TripService {
  public async list(filters: { search?: string; status?: TripStatus }): Promise<Array<ITripDocument & { order_count: number }>> {
    ...
    const counts = await TripOrderModel.aggregate<{ _id: string; count: number }>([
  ...
  public async updateStatus(id: string, status: TripStatus): Promise<ITripDocument | null> {
  ...
  public async getAssignableOrders(search?: string) {
  ...
  public async assignOrders(tripId: string, orderIds: string[]) {
    ...
    await TripOrderModel.insertMany(
      uniqueOrderIds.map((orderId, offset) => ({
        trip: tripId,
        order: orderId,
  ...
  public async getTripOrders(tripId: string) {
    const trip = await TripModel.findById(tripId)
      .populate("city", "name")
      .populate("driver", "name");
    ...
    return {
      trip,
      orders: tripOrders,
      summary: this.calculateSettlement(tripOrders as any[]),
    };
  }

  public async updateTripOrderStatus(
    tripId: string,
    tripOrderId: string,
    payload: { status: TripOrderStatus; fee_applied?: boolean; note?: string },
  ) {
  ...
  public async closeTrip(tripId: string) {
    ...
    const trip = await TripModel.findByIdAndUpdate(
      tripId,
      { status: TripStatus.DELIVERED, delivered_at: new Date() },
      { returnDocument: "after" },
    )
      .populate("city", "name")
      .populate("driver", "name");
  ...
  private calculateSettlement(tripOrders: any[]): TripSettlement {
    const summary: TripSettlement = {
      total_orders: 0,
      delivered_count: 0,
      returned_count: 0,
      damaged_count: 0,
